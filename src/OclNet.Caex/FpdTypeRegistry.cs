...
/// <summary>
/// VDI 3682 / FPD type registry. Resolves the OCL type of the three CAEX object
/// kinds an FPD model uses:
/// <list type="bullet">
///   <item><b>InternalElement</b> — concrete type is the last segment of its
///   <c>RefBaseSystemUnitPath</c> (<c>…/FPD_Product</c> → <c>FPD_Product</c>);</item>
...
/// </list>
/// The abstract supertypes (<c>FPD_State</c>, <c>FPD_Object</c>,
/// <c>FPD_Connection</c>) have no instances of their own and are answered through
/// the hierarchy. Constants mirror the standard's FPD libraries; kept here so the
/// engine stays self-contained (see <see cref="ICaexTypeRegistry"/> for the swap seam).
/// </summary>
public sealed class FpdTypeRegistry : ICaexTypeRegistry
{
    ...
    {
        InternalElementType ie => Known(LocalName(ie.RefBaseSystemUnitPath)),
